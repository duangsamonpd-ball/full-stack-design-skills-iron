 ...
 *
 * --self-test plants a note that must be reported and one that must not.
 */
...
if (process.argv.includes('--self-test')) {
  ...
  console.log(bad ? `SELF-TEST FAILED (${bad})`
    : `SELF-TEST PASSED — ${checks.length} cases: stale detected, active kept, decisions exempt, missing verify caught`);
...
if (!notes.length) { console.log(`no notes in ${DIR}`); process.exit(0); }
